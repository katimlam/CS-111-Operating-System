...
int main(){
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/*3. unallocated inode*/
	int unalloc_inode_size = 0;
	int unalloc_inode_malloced = 64;
	struct block_structure** unalloc_inode = calloc(unalloc_inode_malloced, sizeof(struct block_structure*));

	for (i = 0; i < directory.rows; i++)
	{
		int mismatch = 1;
		for (k = 0; k < inode.rows; k++){
			if (strcmp(directory.table[i][4], inode.table[k][0]) == 0){
				mismatch = 0;
				break;
			}
		}
		if (mismatch == 1){
			bbb = malloc(sizeof(struct block_structure));
			bbb->block = strtoul(directory.table[i][4], NULL, 10);
			bbb->entry = strtoul(directory.table[i][1], NULL, 10);
			bbb->directory = strtoul(directory.table[i][0], NULL, 10);
			bbb->next = NULL;
			unalloc_inode = block_structure_insert(unalloc_inode, bbb, &unalloc_inode_size, &unalloc_inode_malloced);
		}
	}

	for (i = 0; i < unalloc_inode_size; i++){
		struct block_structure* ptr = unalloc_inode[i];
		dprintf(fd, "UNALLOCATED INODE < %lu > REFERENCED BY", ptr->block);
		do
		{
			dprintf(fd, " DIRECTORY < %lu > ENTRY < %lu >", ptr->directory, ptr->entry);
			ptr = ptr->next;
		} while (ptr != NULL);
		dprintf(fd, "\n");
	}
